...
// Lights settings
//directional light
GLfloat light0_position[4] = {0.0f, 0.0f, 5.0f, 1.0f};
...
//point light
GLfloat light1_position[4] = {0.0f, 0.0f, 5.0f, 1.0f};
...
//spot light
GLfloat light2_position[4] = {0.0f, 0.0f, 5.0f, 1.0f};
...
void TextureModel()
{
	...
	while(group){
		...
		for(unsigned int i=0; i<group->numtriangles; i++){
			...
			// the vertex normal
			vnormals[gCount][i+0] = OBJ->normals[indn1*3];
			vnormals[gCount][i+1] = OBJ->normals[indn1*3+1];
			vnormals[gCount][i+2] = OBJ->normals[indn1*3+2];
			vnormals[gCount][i+3] = OBJ->normals[indn2*3];
			vnormals[gCount][i+4] = OBJ->normals[indn2*3+1];
			vnormals[gCount][i+5] = OBJ->normals[indn2*3+2];
			vnormals[gCount][i+6] = OBJ->normals[indn3*3];
			vnormals[gCount][i+7] = OBJ->normals[indn3*3+1];
			vnormals[gCount][i+8] = OBJ->normals[indn3*3+2];	
...
void processMouse(int who, int state, int x, int y){
	glGetUniformfv(p,iLocModelMatrix,tModel);	
	for (int i  = 0;i<16;i++)
		printf("%f \n",tModel[i]);		
	printf("-----------\n");
